...
{
    public partial class _1_comprasEjercicios : System.Web.UI.Page
    {

        panel1Compras p1C = new panel1Compras();
        ...
        {            
            GridView_Administrador.DataSource = p1C.mostrarMisEjercicios(empresa).Skip(pagina * 10).Take(10);
        ...
        public void PaginadoGridViewAdministrador(int empresa) // pregunta en que momento tomo las condiciones del paginado si cuando arranca la pagina o cuando presiono el boton de buscar
        {
            ...
            ViewState["Cantidad_De_Datos"] = p1C.paginadoMisEjercicios(empresa); // cuenta la cantidad de datos
        ...
        public void MostrarGridViewSupervisor(int empresa, int pagina)
        {
            ...
            GridView_Supervisor.DataSource = p1C.mostrarMisEjercicios(empresa).Skip(pagina * 10).Take(10);
        ...
        protected void Identificador_Supervisor(object sender, EventArgs e) // convierto al datalist en editable 
        {
            ...
            List<mostrarMisEjerciciosResult> Datos = p1C.mostrarMisEjercicios(0); // carga los datos del administrador elegido por el supervisor            
            GridViewRow row = GridView_Supervisor.SelectedRow; // defiene la variable row del tipo gridview
            Session["ID_CompraEjercicioExplicaicones"] = GridView_Supervisor.DataKeys[row.RowIndex].Value; // captura el identificador del Administrador seleccionado           

            numeroEjercicio_Supervisor.Text = (Datos[(int)Session["ID_CompraEjercicioExplicaicones"] - 1].id_Ejercicio).ToString(); // carga el administrador de la base
            ejercicioSolucion_Supervisor.Checked = Datos[(int)Session["ID_CompraEjercicioExplicaicones"] - 1].ejercicio;
            ejercicioExplicacion_Supervisor.Checked = Datos[(int)Session["ID_CompraEjercicioExplicaicones"] - 1].explicacion;
            vencimiento_Supervisor.Text = (Datos[(int)Session["ID_CompraEjercicioExplicaicones"] - 1].venvimiento).ToString(); // carga el password de la base                       
            usuario_Supervisor.Text = (Datos[(int)Session["ID_CompraEjercicioExplicaicones"] - 1].id_Usuario).ToString();
            
        }

        protected void Boton_Actualizar_Supervisor_Click(object sender, EventArgs e)
        {
            p1C.actualizarMisEjercicios(Convert.ToInt16(Session["ID_CompraEjercicioExplicaicones"]),ejercicioSolucion_Supervisor.Checked,ejercicioExplicacion_Supervisor.Checked,int.Parse(vencimiento_Supervisor.Text));
            Formulario_Supervisor.Visible = false;
        ...
        protected void Boton_Nuevo_Supervisor_Click(object sender, EventArgs e)
        {
            ...
            if (Boton_Nuevo_Supervisor.Text == "Nuevo")
            {
                ...
                ejercicioSolucion_Supervisor.Checked = false;
                ejercicioExplicacion_Supervisor.Checked = false;
            ...
            if (Boton_Nuevo_Supervisor.Text == "Insertar")
            {
                ...
                int? respuesta = p1C.insertarMisEjercicios(int.Parse(usuario_Supervisor.Text), int.Parse(numeroEjercicio_Supervisor.Text),ejercicioSolucion_Supervisor.Checked,ejercicioExplicacion_Supervisor.Checked,Convert.ToInt16(Session["Empresa"]));
        ...
        {
            p1C.borrarMisEjercicios((int)Session["ID_CompraEjercicioExplicaicones"]);
        ...
        protected void Boton_Excel_Supervisor_Click(object sender, EventArgs e)
        {
            ...
            GridView_Supervisor.DataSource = p1C.mostrarMisEjercicios((int)Session["Empresa"]);
            GridView_Supervisor.DataBind();
            ...
            Response.AddHeader("Content-Disposition", "attachment;filename=Panel_CompraEjercicios.xls");
        ...
        public void MostrarGridViewDios(int empresa, int pagina)
        {
            ...
            GridView_Dios.DataSource = p1C.mostrarMisEjercicios(empresa).Skip(pagina * 10).Take(10);
        ...
        protected void Identificador_Dios(object sender, EventArgs e) // convierto al datalist en editable 
        {
            ...
            Session["ID_CompraEjercicioExplicaicones"] = GridView_Dios.DataKeys[row.RowIndex].Value; // captura el identificador del Administrador seleccionado           

            numeroEjercicio_Dios.Text = (Datos[(int)Session["ID_CompraEjercicioExplicaicones"] - 1].id_Ejercicio).ToString(); // carga el administrador de la base
            ejercicioSolucion_Dios.Checked = Datos[(int)Session["ID_CompraEjercicioExplicaicones"] - 1].ejercicio;
            ejercicioExplicacion_Dios.Checked = Datos[(int)Session["ID_CompraEjercicioExplicaicones"] - 1].explicacion;            
            usuario_Dios.Text = (Datos[(int)Session["ID_CompraEjercicioExplicaicones"] - 1].id_Usuario).ToString();
        ...
        protected void Boton_Excel_Dios_Click(object sender, EventArgs e)
        {
            ...
            GridView_Supervisor.EnableViewState = false;
            GridView_Supervisor.AllowPaging = false;
            GridView_Supervisor.DataSource = p1C.mostrarMisEjercicios(0);
            GridView_Supervisor.DataBind();
            page.EnableEventValidation = false;
            page.DesignerInitialize();
            page.Controls.Add(form);
